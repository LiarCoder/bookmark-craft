...
- 需要更短的书签栏名称时，使用可选的 `// @bookmark-name: 简短名称`，名称不超过 6 个字符；未声明时沿用展示名
- `public/bookmarklets.json` 是受版本控制的静态页面数据，禁止手工编辑
- 修改源码后运行 `npm run build`，同时提交源码与变化的静态清单
- 新增或删除书签时保持 `src/bookmarklets/`、`docs/` 同名对应，并重新生成静态清单
...
- 独立主题分开提交；源码与对应静态清单放在同一提交

## 当前状态

核心构建、静态页面、CLI 安全和页面脚本已有 Node 内置测试。项目没有既定功能路线图，后续只围绕明确需求或可复现缺陷演进。
